...
const SaleModal = ({ show, setShow }: PropType) => {
  ...
  const AddSaleMutation = useMutation({
    ...
    onSuccess: (data) => {
      dispatch(notify({ type: "success", message: "Product Sold" }));
  ...
  const isLoading = AddSaleMutation.isPending;
  const {
    register,
    handleSubmit,
    control,
    ...
    <>
      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Make a new sale</Modal.Title>
        </Modal.Header>
        <Modal.Body>
